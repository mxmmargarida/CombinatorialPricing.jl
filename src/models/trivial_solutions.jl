...
function toll_free_cost(prob::PricingProblem; silent=true, threads=1)
    tollfree = toll_free_solution(prob; silent, threads)
    costs = base_costs(prob)
    return sum(costs[collect(tollfree)]; init=0.)
end

function null_toll_cost(prob::PricingProblem; silent=true, threads=1)
    nulltoll = null_toll_solution(prob; silent, threads)
    costs = base_costs(prob)
    return sum(costs[collect(nulltoll)]; init=0.)
end

function difference_estimate(prob::PricingProblem; silent=true, threads=1)
    tollfree = toll_free_cost(prob; silent, threads)
    nulltoll = null_toll_cost(prob; silent, threads)
    return tollfree - nulltoll
end
